<script>
  ...
  import {
    hovered as eHovered,
    selected as eSelected,
  } from "../stores/eventSelections";
  ...
  function handleCentroidClick(event) {
    ...
    if (
      $disinformantNationFilter.filter((d) => d.selected).length ===
      $disinformantNationFilter.length
    ) {
      selectAllFilters();
  ...
  $: sources = timePoints
      ...
      d.disinformantNation.map((disNation, i) => {
        ...
        if (countryNames.includes(disNation)) {
          if ($switchValueStore === "on") {
            coords = $geoPath.centroid(
              $pangeaRegions.find(
                (country) => country.properties.name === disNation
              )
            );
          } else {
            coords = $geoPath.centroid(
            ...
            if (disNation === "North America") {
              coords = [
                $geoPath.centroid(
                  $countries.find(
                    (country) => country.properties.name === "Mexico"
                  )
                )[0],
                $geoPath.centroid(
                  $countries.find(
                    (country) => country.properties.name === "Spain"
                  )
                )[1],
              ];
            }
    ...
    .map((d, i, arr) => {
      ...
      if (arr[i - 1] && arr[i - 1].disNation === d.disNation) {
        shift =
          d.xCountry < d._x
            ? (disNationNumberRight += 1)
            : (disNationNumberLeft -= 1);
      } else {
    ...
    .map((d, _, arr) => {
      const minDisNationShift = Math.min(
        ...arr.filter((a) => a.disNation === d.disNation).map((a) => a.shift)
      );
      return {
        ...d,
        shift:
          d.shift <= 0 ? -1 - (d.shift + Math.abs(minDisNationShift)) : d.shift,
      };
    });

  $: centroids = [
    ...group(
      sources.filter((s) => s.nativeCountry),
      (s) => s.disNation
    ),
  ];
</script>
...
<g class="centroids-sources" class:hidden={$sourcesSwitch}>
  ...
  {#each centroids as [country, centroid]}
    <Centroid
      {centroid}
      {country}
      selected={$disinformantNationFilter
        .filter((d) => d.selected)
        .map((d) => d.id)
        .includes(country)}
