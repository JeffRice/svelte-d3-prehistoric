<script>
  ...
  let mScale = scaleLinear().domain([-40, 160]).range([0, 1]);
...
>
  {#if chartWidth}
    ...
    >
      <ChartXAxis scale={xScale} y={chartHeight - padding} />
      <ChartYAxis scale={yScale} x={padding} />
      {#each renderedData as { x, y, name, stroke, hoverName, size, weight }}
        <ChartDatapoint {x} {y} {name} {hoverName} {stroke} {weight} {size} />
      {/each}
    </svg>
  {/if}
</div>
