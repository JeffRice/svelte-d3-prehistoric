...
{#if name}
  {#if x <= $width / 2}
    <!-- label-->
    <g
      class="centroid-name-label place-label"
      transform="translate({$tX + 10} {$tY - 20})"
    >
      <text
        class="chart-text"
        fill={stroke}
        style="transition: opacity 600ms ease;">{hoverName}</text
      >
    </g>
  {/if}
  ...
  {#if x > $width / 2}
    ...
    <g
      class="centroid-name-label place-label"
      transform="translate({$tX - 100} {$tY - 20})"
    >
      <text
        class="chart-text"
        fill={stroke}
