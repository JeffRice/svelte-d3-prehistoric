<script>
  ...
  $: if (
    $fossilDatapoints &&
    $fossilDatapoints.originaltriassic &&
    $fossilDatapoints.originaljurassic &&
    $fossilDatapoints.originalcretaceous &&
    data
  ) {
    ...
    jurassicFilter = allNames.map((d, i) => [
      allNames[i],
      $fossilDatapoints.originaljurassic.filter((d) =>
        d.name.includes(allNames[i])
      ),
    ]);
    sortedJurassicFilter = sortBy(jurassicFilter, [
      function (d) {
        return d[1];
      },
    ]).reverse();
    ...
    triassicFilter = allNames.map((d, i) => [
      allNames[i],
      $fossilDatapoints.originaltriassic.filter((d) =>
        d.name.includes(allNames[i])
      ),
    ]);

    sortedTriassicFilter = sortBy(triassicFilter, [
      function (d) {
        return d[1];
      },
    ]).reverse();
    uniqueTriassicNames = uniq(
      $fossilDatapoints.originaltriassic.map((d) => d.name)
    );
    sortedTriassicNames = sortBy(uniqueTriassicNames.map((d) => d)).join(" | ");

    cretaceousFilter = allNames.map((d, i) => [
      allNames[i],
      $fossilDatapoints.originalcretaceous.filter((d) =>
        d.name.includes(allNames[i])
      ),
    ]);

    sortedCretaceousFilter = sortBy(cretaceousFilter, [
      function (d) {
        return d[1];
      },
    ]).reverse();
    uniqueCretaceousNames = uniq(
      $fossilDatapoints.originalcretaceous.map((d) => d.name)
    );
...
<div class="choice-controls">
  ...
  <button
    ...
    on:click|stopPropagation={removeAllFossils}>remove all</button
  >
  <button
    class="choice-controls-unselectall"
    on:click|stopPropagation={addAllFossils}>add all</button
  >
</div>
    ...
    <section class="fossil-wrap">
      ...
      {#each sortedCretaceousFilter as fossilDatapoint, i}
        {#if fossilDatapoint[1].length}
          <section class="fossil-names">
            {fossilDatapoint[0]} ({fossilDatapoint[1].length})<br />
            <button
              class="CretaceousButton"
              on:click={() => addDino(fossilDatapoint[1], "cretaceous")}
            >
              Plot {fossilDatapoint[0]} fossils
            </button><br />
            <input
              id="collapsible-Cretaceous-{i}"
              class="toggle"
              type="checkbox"
            />
            <label for="collapsible-Cretaceous-{i}" class="lbl-toggle top"
              >View Locations</label
            >
            <div class="collapsible-content">
              {#each fossilDatapoint[1] as newTest, i}
                <p>
                  Dinosaur Name: {newTest.name}, Coords:({newTest.x}, {newTest.y})
                </p>
      ...
      {#each sortedJurassicFilter as fossilDatapoint, i}
        {#if fossilDatapoint[1].length}
          <section class="fossil-names">
            {fossilDatapoint[0]} ({fossilDatapoint[1].length})<br />
            <button
              class="JurassicButton"
              on:click={() => addDino(fossilDatapoint[1], "jurassic")}
            >
              Plot {fossilDatapoint[0]} fossils
            </button><br />
            <input
              id="collapsible-Jurassic-{i}"
              class="toggle"
              type="checkbox"
            />
            <label for="collapsible-Jurassic-{i}" class="lbl-toggle top"
              >View Locations</label
            >
            <div class="collapsible-content">
              {#each fossilDatapoint[1] as newTest, i}
      ...
      {#each sortedTriassicFilter as fossilDatapoint, i}
        {#if fossilDatapoint[1].length}
          <section class="fossil-names">
            {fossilDatapoint[0]} ({fossilDatapoint[1].length})<br />
            <button
              class="TriassicButton"
              on:click={() => addDino(fossilDatapoint[1], "triassic")}
            >
              Plot {fossilDatapoint[0]} fossils
            </button><br />
            <input
              id="collapsible-Triassic-{i}"
              class="toggle"
              type="checkbox"
            />
            <label for="collapsible-Triassic-{i}" class="lbl-toggle top"
              >View Locations</label
            >
            <div class="collapsible-content">
              {#each fossilDatapoint[1] as newTest, i}
...
<button on:click|stopPropagation={showMoreDetails} id="flex-show"
  >Fossils Continued... <i class="arrow down" /></button
>
...
{#if sortedCretaceousNames && sortedJurassicNames && sortedTriassicNames}
  <section class="unique-fossils">
    <section class="fossil-wrap">
      <h3 class="Cretaceous">
        Unique Cretaceous Dinosaur Names ({uniqueCretaceousNames.length})
      </h3>
      <input
        id="collapsible-Cretaceous-unique"
        class="toggle"
        type="checkbox"
      />
      <label for="collapsible-Cretaceous-unique" class="lbl-toggle top"
        >View All Fossil Names</label
      >
      <div class="collapsible-content">
        {sortedCretaceousNames}
      </div>
    ...
    <section class="fossil-wrap">
      <h3 class="Jurassic">
        Unique Jurassic Dinosaur Names ({uniqueNames.length})
      </h3>
      <input id="collapsible-Jurassic-unique" class="toggle" type="checkbox" />
      <label for="collapsible-Jurassic-unique" class="lbl-toggle top"
        >View All Fossil Names</label
      >
      <div class="collapsible-content">
        {sortedJurassicNames}
    ...
    <section class="fossil-wrap">
      <h3 class="Triassic">
        Unique Triassic Dinosaur Names ({uniqueTriassicNames.length})
      </h3>
      <input id="collapsible-Triassic-unique" class="toggle" type="checkbox" />
      <label for="collapsible-Triassic-unique" class="lbl-toggle top"
        >View All Fossil Names</label
      >
      <div class="collapsible-content">
        {sortedTriassicNames}
