<script>
  ...
  $: if (chartData.a) {
    selectedData = chartData[selectedOption].sort((a, b) =>
      a.x < b.x ? 1 : -1
    );
  }
          ...
          if (this != that) {
            ...
            if (
              ...
              var dx =
                  ...
                  0.01,
                dy =
                  (Math.max(1, a.bottom - b.top) +
                    Math.min(1, a.top - b.bottom)) *
                  0.02,
...
{#if selectedData}
  <div class="wrapper">
    <Chart
      data={selectedData}
      xRange={[
        min(chartData[selectedOption].map((d) => d.x)),
        max(chartData[selectedOption].map((d) => d.x)),
      ]}
      yRange={[
        min(chartData[selectedOption].map((d) => d.y)),
        max(chartData[selectedOption].map((d) => d.y)),
      ]}
    />

    <section class="chart-header">
      Prehistoric Life - Size and Weight distribution
    </section>
